...
我把一个卷积层+一个激活函数+一个最大池化+一个批归一化层作为一个大层，这样的大层来4层，后面接三个全连接层和一个dropout层。
...
这里我设置了100个epoch，每个epoch中有训练数据，在验证集上测试和在测试集上测试。训练数据有以下步骤：按batch读入数据，梯度清零，生成输出，计算loss，反向传播优化参数，优化学习率。
...
​	虽然说我的神经网络最后无论是在验证集还是测试集上的准确率都已经“看起来”稳定了，但其实训练Loss还没有收敛，在2.7左右反复横跳，陷入了梯度消失的情况。
